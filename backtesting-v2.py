...
Algorithm: SMA Momentum Crossover on VN30F1M
- Buy signal:  previous tick < SMA AND current tick >= SMA  → Limit Buy at ceiling price
- Sell signal: previous tick > SMA AND current tick <= SMA  → Limit Sell at floor price
- Take-Profit: unrealized P&L >= +3 points
- Stop-Loss:   unrealized P&L <= -2 points
...
class Backtesting:
    """
    Backtesting class — SMA Momentum Crossover strategy.

    Rules (per the CS408 algorithm spec):
    - Signal generated by SMA of tick prices.
    ...
    - After TP/SL exit, the algorithm immediately looks for the next crossover
      to re-enter, allowing multiple round-trip trades per day.
    """
    ...
    def _current_sma(self) -> Optional[Decimal]:
        """Return SMA if enough data, else None."""
    ...
    def run(self, data: pd.DataFrame, step: Decimal = Decimal('0.1')):
        """
        Iterate tick-by-tick through `data` and apply the SMA crossover strategy.
        ...
        for index, row in data.iterrows():
            ...
            if cur_sma is not None and self._prev_price is not None and self._prev_sma is not None:
                ...
                # This fires both when flat (after TP/SL) and when no position exists,
                # allowing immediate re-entry on the same tick after a TP/SL close.
                if self.inventory == 0:
            ...
            if is_last_tick_of_day:
                ...
                if self.printable:
                    print(
                        ...
                        f"Total trades so far: {self.total_trades}"
                    )
                ...
                # reset daily counters — keep SMA window and prev tick
                # across days so signals can fire from the very first tick
                moving_to_f2 = False
                cur_index += 1
...
if __name__ == "__main__":
    ...
    num_days = len(bt.tracking_dates)
    print(f"\n--- Trade Summary ---")
    print(f"Total trades: {bt.total_trades}")
    print(f"Total trading days: {num_days}")
    print(f"Trades per day: {bt.total_trades / num_days:.2f}")
    print(f"Trades per 10 days: {bt.total_trades / num_days * 10:.1f}")
    print()
